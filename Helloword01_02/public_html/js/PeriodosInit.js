...
app.controller("pagina", function ($scope, $filter, $http) {
        ...
        $http.get('/api/Periodos/').then(function (d) {
            ...
            $scope.cambiarPeriodo = function (i) {
                $http.get('/api/Evaluacion/' + $scope.items[i].id_evaluacion).then(function (d) {
                    ...
                    $scope.nombre_periodo_update = $scope.items[i].nombre;
                    $scope.fecha_fin_update = $scope.items[i].fechafinal.split('T')[0];
                    $scope.fecha_inicio_update = $scope.items[i].fechainicio;
                    $scope.metodologia_update = $scope.items[i].metodologia;
                    $("#query_update").val(d.data.nombre); // debo traer el nombre
    ...
    $scope.RegistrarPeriodo = function () {
        ...
        $http.get('/Evaluacion/' + document.getElementById('query').value).then(function (d) {
            if (d.data.nombre !== null) {
                var periodo = {
                    id: null,
                    fechainicio: new Date().getFullYear() + "-" + (new Date().getMonth() + 1) + "-" + new Date().getDate(),
                    fechafinal: $scope.fecha_fin,
                    metodologia: $scope.metodologia,
                    id_evaluacion: d.data.id,
                    estado: 1,
                    nombre: $scope.nombre
                };

                // luego añado las validaciones
                $http.post('/api/Periodos/', periodo).then(function (d) {
                    console.log(d.data);
    ...
    $scope.UpdatePeriodo = function () {
        $http.get('/Evaluacion/' + document.getElementById('query_update').value).then(function (d) {
            if (d.data.nombre !== null) {
                var periodo = {
                    id: $scope.id_pudate,
                    fechainicio: $scope.fecha_inicio_update,
                    fechafinal: $scope.fecha_fin_update,
                    metodologia: $scope.metodologia_update,
                    id_evaluacion: d.data.id,
                    estado: $scope.estado_update,
                    nombre: $scope.nombre_periodo_update
                };

                console.log(periodo);
                $http.put('/api/Periodos/' + periodo.id, periodo).then(function () {
                    // refrescamos
                    $scope.iniciar();
                });
            }
